...
uint8_t TrxNetwork::begin(uint8_t type_network)
{
	...
	gui.updateBottomStatus(LV_COLOR_ORANGE, sString);
	...
	gui.updateBottomStatus(LV_COLOR_NAVY, sString);
	gui.showWifilabel(false);
	init_ota();
...
ArduinoOTA.onStart([]() {
	...
	if (ArduinoOTA.getCommand() == U_FLASH)
		...
		type = "filesystem";
	gui.show_ota_gui(true);
	gui.set_ota_label(String("Start OTA: updating ") + type);
	})
	.onEnd([]() {
		gui.set_ota_label1("End");
		delay(1000);
		})
		.onProgress([](unsigned int progress, unsigned int total) {
			...
			gui.set_ota_label1("Progress: " + String((progress / (total / 100))));
			})
			.onError([](ota_error_t error) {
				gui.set_ota_label("Error : " + String(error));
				delay(1000);
				if (error == OTA_AUTH_ERROR) gui.set_ota_label1("Auth Failed"); 
				else if (error == OTA_BEGIN_ERROR) gui.set_ota_label1("Begin Failed");
				else if (error == OTA_CONNECT_ERROR) gui.set_ota_label1("Connect Failed");
				else if (error == OTA_RECEIVE_ERROR) gui.set_ota_label1("Receive Failed");
				else if (error == OTA_END_ERROR) gui.set_ota_label1("End Failed");
				});
